...
import{RequestsPage,ExecutiveDashboard,LeavePage}from'./OperationsModules';
...
function storageObjectPath(bucket,value){
 const raw=String(value||'').replace(/^IMAGE::/,'');
 const marker='/storage/v1/object/public/'+bucket+'/';
 const index=raw.indexOf(marker);
 if(index<0)return null;
 try{return decodeURIComponent(raw.slice(index+marker.length).split('?')[0])}catch{return raw.slice(index+marker.length).split('?')[0]}
}
async function removeStorageLinks(bucket,values){
 const paths=[...new Set((values||[]).map(value=>storageObjectPath(bucket,value)).filter(Boolean))];
 if(!paths.length)return;
 const{error}=await supabase.storage.from(bucket).remove(paths);
 if(error)console.error('Không xóa được file Storage:',bucket,error);
}
function App(){
 ...
 async function loadBoard(boardIdOverride,branchFilter){
  ...
  if(boardError){console.error('Không tải được Board:',boardError);setBoards([]);return;}
  const allBoards=bd||[];
  setBoards(allBoards);
  const branchBoards=branchFilter?allBoards.filter(b=>b.branch_id===branchFilter):allBoards;
  const defaultBoard=(branchBoards[0]||allBoards[0])?.id||'';
  const targetBoard=boardIdOverride||(selectedBoard&&allBoards.some(b=>b.id===selectedBoard)?selectedBoard:null)||defaultBoard;
  if(!targetBoard){setBoardLists([]);setBoardCards([]);setCardChecks([]);setCardComments([]);return;}
  if(targetBoard!==selectedBoard)setSelectedBoard(targetBoard);
  ...
  const[{data:bl,error:listError},{data:bc,error:cardError}]=await Promise.all([
   ...
   supabase.from('board_cards').select('id,board_id,list_id,title,description,cover_image,owner_id,due_date,label,recurrence_cycle,position,created_by,created_at,updated_at,workflow_status,started_at,submitted_at,reviewed_at,reviewer_id,review_note,result_url,base_kpi_points,earned_kpi_points,revision_count').eq('board_id',targetBoard).order('position',{ascending:true}).order('created_at',{ascending:true}).limit(500)
  ]);
  if(requestId!==boardLoadVersion.current)return;
 ...
 useEffect(()=>{if(session?.user&&page==='BOARD'){const bf=isStaff?profile?.branch_id:undefined;loadBoard(selectedBoard||undefined,bf)}},[page,session?.user?.id]);
 useEffect(()=>{if(!session?.user)return;
  ...
  const reloadBoardStructure=()=>{if(page!=='BOARD')return;clearTimeout(boardDebounceTimer);boardDebounceTimer=setTimeout(()=>loadBoard(selectedBoard||undefined),600);};
  const applyCardChange=payload=>{const row=payload.new||payload.old||{};if(payload.eventType==='DELETE'){setBoardCards(prev=>prev.filter(x=>x.id!==row.id));return;}if(row.board_id!==selectedBoard)return;setBoardCards(prev=>payload.eventType==='INSERT'?(prev.some(x=>x.id===row.id)?prev:[...prev,row]):prev.map(x=>x.id===row.id?{...x,...row}:x));};
  const applyChecklistChange=payload=>{const row=payload.new||payload.old||{};if(!row.id)return;if(payload.eventType==='DELETE'){setCardChecks(prev=>prev.filter(x=>x.id!==row.id));return;}setCardChecks(prev=>{const exists=prev.some(x=>x.id===row.id);if(payload.eventType==='INSERT')return exists?prev:[...prev,row].sort((a,b)=>(a.position||0)-(b.position||0));return exists?prev.map(x=>x.id===row.id?{...x,...row}:x):prev;});};
  const applyCommentChange=payload=>{const row=payload.new||payload.old||{};if(payload.eventType==='DELETE'){setCardComments(prev=>prev.filter(x=>x.id!==row.id));return;}setCardComments(prev=>payload.eventType==='INSERT'?(prev.some(x=>x.id===row.id)?prev:[row,...prev]):prev.map(x=>x.id===row.id?{...x,...row}:x));};
  const refreshVisibleBoard=()=>{if(document.visibilityState==='visible')reloadBoardStructure();};
  const scheduleDailyChecklistRefresh=()=>{const now=Date.now(),vnNow=new Date(now+7*60*60*1000);let next=Date.UTC(vnNow.getUTCFullYear(),vnNow.getUTCMonth(),vnNow.getUTCDate(),14,0,30);if(next<=now)next+=24*60*60*1000;return setTimeout(()=>{reloadBoardStructure();dailyChecklistTimer=scheduleDailyChecklistRefresh();},next-now);};
  let dailyChecklistTimer=scheduleDailyChecklistRefresh();
  window.addEventListener('focus',reloadBoardStructure);
  document.addEventListener('visibilitychange',refreshVisibleBoard);
  const c=supabase.channel('akc-v11').on('postgres_changes',{event:'*',schema:'public',table:'leads'},reloadLeads).on('postgres_changes',{event:'*',schema:'public',table:'profiles'},reloadProfiles).on('postgres_changes',{event:'*',schema:'public',table:'branches'},reloadBranches).on('postgres_changes',{event:'*',schema:'public',table:'operation_tasks'},reloadTasks).on('postgres_changes',{event:'*',schema:'public',table:'pt_checklists'},reloadPtShows).on('postgres_changes',{event:'*',schema:'public',table:'boards'},reloadBoardStructure).on('postgres_changes',{event:'*',schema:'public',table:'board_lists'},reloadBoardStructure).on('postgres_changes',{event:'*',schema:'public',table:'board_cards'},applyCardChange).on('postgres_changes',{event:'*',schema:'public',table:'card_checklists'},applyChecklistChange).on('postgres_changes',{event:'*',schema:'public',table:'card_comments'},applyCommentChange).subscribe();return()=>{clearTimeout(boardDebounceTimer);clearTimeout(dailyChecklistTimer);window.removeEventListener('focus',reloadBoardStructure);document.removeEventListener('visibilitychange',refreshVisibleBoard);supabase.removeChannel(c)}},[session?.user?.id,page,selectedBoard]);
 ...
 const isAdmin=profile.role==='admin',isManager=profile.role==='manager',isSale=profile.role==='sale',isPT=profile.role==='pt',isReception=profile.role==='reception',isStaff=['sale','pt','reception'].includes(profile.role);
 ...
 const nav=isReception?['BOARD','Nghỉ phép']:isPT?['BOARD','Show PT','Kế Hoạch','Nghỉ phép']:['Tổng quan','Lead','Pipeline','Lịch hẹn','BOARD',...(isAdmin||isManager?['Đề xuất']:[]),'Nghỉ phép','Show PT','Kế Hoạch',...(isAdmin||isManager?['Báo cáo']:[]),...(isAdmin?['Cơ sở',`Tài khoản${pending?' ('+pending+')':''}`]:[]),'Zalo/Facebook'];
 ...
 async function delBoard(id){if(!(isAdmin||isManager))return alert('Chỉ admin/manager được xóa board');if(confirm('Xóa board này và toàn bộ danh sách/thẻ?')){const cards=boardCards.filter(c=>c.board_id===id);const comments=cardComments.filter(c=>cards.some(card=>card.id===c.card_id));const{error}=await supabase.from('boards').delete().eq('id',id);if(error)return alert(error.message);await Promise.all([removeStorageLinks('card-images',cards.map(c=>c.cover_image)),removeStorageLinks('card-comments',comments.map(c=>c.message))]);setSelectedBoard('');loadBoard()}}
 async function quickBoard(){const{data:b,error}=await supabase.from('boards').insert({name:'Ban Điều Hành AKC',background:DEFAULT_BOARD_BG,position:boards.length,created_by:profile.id}).select().single();if(error)return alert(error.message);setSelectedBoard(b.id);const names=['Giám Đốc Điều Hành','Phòng chất lượng PT','Leader Team LT','Phòng Kinh Doanh'];const rows=names.map((name,i)=>({board_id:b.id,name,position:i,created_by:profile.id}));const{data:ls,error:le}=await supabase.from('board_lists').insert(rows).select();if(le)return alert(le.message);if(ls?.[0])await supabase.from('board_cards').insert({board_id:b.id,list_id:ls[0].id,title:'CEO',description:'Mô tả công việc và quyền hạn điều hành.\n- Xây dựng chiến lược\n- Theo dõi KPI\n- Duyệt kế hoạch',position:0,created_by:profile.id,owner_id:profile.id});loadBoard()}
 async function addBoardList(boardId,name){if(!boardId)return alert('Chưa chọn board');if(!name.trim())return;const pos=boardLists.filter(l=>l.board_id===boardId).length;const{error}=await supabase.from('board_lists').insert({board_id:boardId,name:name.trim(),position:pos,created_by:profile.id});if(error)alert(error.message);loadBoard()}
 async function renameBoardList(list){const name=prompt('Tên danh sách',list.name);if(name){const{error}=await supabase.from('board_lists').update({name}).eq('id',list.id);if(error)alert(error.message);loadBoard()}}
 async function delBoardList(id){if(confirm('Xóa danh sách và các thẻ bên trong?')){const cards=boardCards.filter(c=>c.list_id===id);const comments=cardComments.filter(c=>cards.some(card=>card.id===c.card_id));const{error}=await supabase.from('board_lists').delete().eq('id',id);if(error)return alert(error.message);await Promise.all([removeStorageLinks('card-images',cards.map(c=>c.cover_image)),removeStorageLinks('card-comments',comments.map(c=>c.message))]);loadBoard()}}
 async function saveBoardCard(f){const boardId=f.board_id||selectedBoard;if(!boardId)return alert('Chưa chọn board');let listId=f.list_id;if(!listId){const first=boardLists.find(l=>l.board_id===boardId);if(!first)return alert('Board chưa có danh sách');listId=first.id}const row={board_id:boardId,list_id:listId,title:f.title||'Thẻ mới',description:f.description||'',cover_image:f.cover_image||'',owner_id:f.owner_id||null,due_date:f.due_date||null,label:f.label||'',recurrence_cycle:['none','daily','weekly','monthly'].includes(f.recurrence_cycle)?f.recurrence_cycle:'none',base_kpi_points:Math.max(0,Number(f.base_kpi_points||30)),reviewer_id:f.reviewer_id||null,position:Number(f.position||0),updated_at:new Date().toISOString()};const q=f.id?supabase.from('board_cards').update(row).eq('id',f.id):supabase.from('board_cards').insert({...row,created_by:profile.id}).select().single();const{data,error}=await q;if(error)return alert(error.message);const cardId=f.id||data?.id;if(cardId&&f.checklist_text!==undefined){const lines=String(f.checklist_text||'').split(/[\n.;]+/).map(x=>x.trim()).filter(Boolean);if(lines.length){// Fix 1: Query DB trực tiếp thay vì dùng state để tránh stale data gây mất checklist
const{data:oldChecks}=await supabase.from('card_checklists').select('text,position').eq('card_id',cardId);const old=oldChecks||[];const newLines=lines.filter(l=>!old.find(o=>String(o.text).trim().toLowerCase()===l.toLowerCase()));if(newLines.length){const maxPos=old.length?Math.max(...old.map(o=>o.position||0))+1:0;await Promise.all(newLines.map((text,i)=>supabase.from('card_checklists').insert({card_id:cardId,text,position:maxPos+i,created_by:profile.id})));}}}setCardModal(null);loadBoard()}
 ...
 async function toggleCheck(id,done){setCardChecks(prev=>prev.map(c=>c.id===id?{...c,done:!done}:c));const{error}=await supabase.from('card_checklists').update({done:!done}).eq('id',id);if(error){alert(error.message);setCardChecks(prev=>prev.map(c=>c.id===id?{...c,done:done}:c));}}
 async function updateCheckText(id,text){if(!String(text||'').trim())return alert('Nội dung checklist không được trống');const trimmed=String(text).trim();setCardChecks(prev=>prev.map(c=>c.id===id?{...c,text:trimmed}:c));const{error}=await supabase.from('card_checklists').update({text:trimmed}).eq('id',id);if(error)alert(error.message);}
 async function deleteCheck(id){if(!confirm('Xóa mục checklist này?'))return;const prev=cardChecks;setCardChecks(c=>c.filter(x=>x.id!==id));const{error}=await supabase.from('card_checklists').delete().eq('id',id);if(error){alert(error.message);setCardChecks(prev);}}
 async function addComment(cardId,message){if(!String(message||'').trim())return;const msg=message.trim();const tmpId='tmp_'+Date.now();const tmp={id:tmpId,card_id:cardId,user_id:profile.id,message:msg,created_at:new Date().toISOString()};setCardComments(prev=>[tmp,...prev]);const{data,error}=await supabase.from('card_comments').insert({card_id:cardId,user_id:profile.id,message:msg}).select().single();if(error){alert(error.message);setCardComments(prev=>prev.filter(c=>c.id!==tmpId));}else{setCardComments(prev=>prev.map(c=>c.id===tmpId?data:c));}}
 async function updateComment(id,message){if(!String(message||'').trim())return alert('Bình luận không được trống');const trimmed=String(message).trim();setCardComments(prev=>prev.map(c=>c.id===id?{...c,message:trimmed}:c));const{error}=await supabase.from('card_comments').update({message:trimmed}).eq('id',id);if(error)alert(error.message);}
 async function deleteComment(id){if(!confirm('Xóa bình luận này?'))return;const prev=cardComments;const comment=prev.find(c=>c.id===id);setCardComments(c=>c.filter(x=>x.id!==id));const{error}=await supabase.from('card_comments').delete().eq('id',id);if(error){alert(error.message);setCardComments(prev);return;}await removeStorageLinks('card-comments',[comment?.message]);}
 async function boardWorkflow(action,card,payload={}){let call;if(action==='start')call=supabase.rpc('start_board_card',{card_id:card.id});if(action==='submit')call=supabase.rpc('submit_board_card',{card_id:card.id,result_link:payload.result_url||''});if(action==='approve')call=supabase.rpc('review_board_card',{card_id:card.id,approved:true,note:payload.note||''});if(action==='redo')call=supabase.rpc('review_board_card',{card_id:card.id,approved:false,note:payload.note||''});const{error}=await call;if(error)return alert(error.message);setCardModal(null);loadBoard(selectedBoard)}
 ...
 const base=isReception&&!['BOARD','Nghỉ phép'].includes(page)?'BOARD':(isPT&&!['BOARD','Show PT','Kế Hoạch','Nghỉ phép'].includes(page)?'BOARD':(page.startsWith('Tài khoản')?'Tài khoản':page));
 return <div className='app'><aside><div className='brand'><div className='logo'>AKC</div><div><h1>AKC CRM</h1><p>Cloud v8</p></div></div><div className='mobile-user'><button className='user-pill' onClick={()=>setUserMenu(!userMenu)}><span>{(profile.full_name||'U').slice(0,1).toUpperCase()}</span></button>{userMenu&&<div className='user-menu'><strong>{profile.full_name}</strong><small>{profile.role}</small><button onClick={()=>setPasswordModal({})}>Đổi mật khẩu</button><button onClick={()=>supabase.auth.signOut()}>Đăng xuất</button></div>}</div><nav>{nav.map(n=><button key={n} className={base===n.split(' (')[0]?'active':''} onClick={()=>setPage(n.split(' (')[0])}>{n}</button>)}</nav><div className='me'><b>{profile.full_name}</b><span>{profile.role} {profile.branch_id?'· '+bname(branches,profile.branch_id):''}</span><button onClick={()=>setPasswordModal({})}>Đổi mật khẩu</button><button onClick={()=>supabase.auth.signOut()}>Đăng xuất</button></div></aside><main><header><div><h2>{base}</h2><p>{isAdmin?'Admin toàn hệ thống':isManager?'Manager cơ sở':isReception?'Lễ tân':isPT?'PT':'Sale cá nhân'} · Supabase realtime</p></div><div><input ref={fileInputRef} type='file' accept='.csv' style={{display:'none'}} onChange={importData}/>{(isAdmin||isManager)&&['Tổng quan','Lead','Pipeline','Lịch hẹn','Báo cáo'].includes(base)&&<><button onClick={()=>fileInputRef.current?.click()}>Nhập CSV</button><button onClick={exportData}>Xuất CSV</button></>}{base==='BOARD'&&!isStaff?<button className='primary' onClick={()=>setBoardModal({})}>+ Tạo board</button>:base==='Lịch hẹn'?<button className='primary' onClick={()=>setCalendarModal({})}>+ Thêm lịch hẹn</button>:base==='Show PT'?<button className='primary' onClick={()=>setModal({type:'teaching_show'})}>+ Đăng ký show PT</button>:['Tổng quan','Lead','Pipeline','Báo cáo'].includes(base)?<button className='primary' onClick={()=>setModal({})}>+ Thêm lead</button>:null}</div></header>{!isPT&&['Tổng quan','Lead','Pipeline','Lịch hẹn','Báo cáo'].includes(base)&&<Filters filter={filter} setFilter={setFilter} branches={avBranches} users={avUsers}/>} {base==='Tổng quan'&&<Dashboard leads={rows} branches={avBranches} users={avUsers}/>} {base==='Lead'&&<Lead leads={rows} branches={branches} users={users} edit={setModal} del={delLead} isAdmin={isAdmin}/>} {base==='Pipeline'&&<Pipeline leads={rows} branches={branches} users={users} setStatus={status}/>} {base==='Lịch hẹn'&&<Calendar leads={rows} branches={branches} users={users} add={()=>setCalendarModal({})}/>} {base==='BOARD'&&<BoardPage boards={visibleBoards} branches={branches} lists={boardLists} cards={boardCards} checks={cardChecks} comments={cardComments} users={users} profile={profile} selectedBoard={selectedBoard} setSelectedBoard={setSelectedBoard} setBoardModal={setBoardModal} setCardModal={openCardModal} addList={addBoardList} renameList={renameBoardList} delList={delBoardList} delBoard={delBoard} moveCard={moveBoardCard} dragCard={dragCard} reloadBoardById={loadBoard}/>} {base==='Đề xuất'&&(isAdmin||isManager)&&<RequestsPage profile={profile} branches={avBranches}/>} {base==='Nghỉ phép'&&<LeavePage profile={profile} branches={avBranches} users={avUsers}/>} {base==='Show PT'&&<TeachingShows shows={ptShows} branches={branches} users={users} profile={profile} edit={setModal} complete={completePtShow} approve={approvePtShow} reject={rejectPtShow} markInvoice={markKiotInvoice} deleteShow={deletePtShow}/>} {base==='Báo cáo'&&<><Reports leads={rows} branches={branches} users={users}/>{(isAdmin||isManager)&&<ExecutiveDashboard profile={profile} branches={avBranches} users={avUsers}/>}</>}  {base==='Cơ sở'&&isAdmin&&<Branches branches={branches} reload={()=>load(true)}/>} {base==='Tài khoản'&&isAdmin&&<Users users={users} branches={branches} reload={()=>load(true)}/>} {base==='Zalo/Facebook'&&<Panel title='Zalo/Facebook' text='Sẵn sàng nối Zalo OA, Facebook Lead Form, Hotline ở bản tiếp theo.'/>} {base==='Kế Hoạch'&&<TrainingPlanWizard profile={profile}/>}</main>{modal&&(modal.type==='teaching_show'?<PtShowModal initial={modal} branches={avBranches} users={users.filter(u=>u.role==='pt'||u.role==='manager'||u.role==='admin')} profile={profile} onClose={()=>setModal(null)} onSave={savePtShow}/>:<LeadModal initial={modal} branches={avBranches} users={avUsers} profile={profile} onClose={()=>setModal(null)} onSave={saveLead}/>)} {calendarModal&&<AppointmentModal initial={calendarModal} branches={avBranches} users={avUsers} profile={profile} onClose={()=>setCalendarModal(null)} onSave={saveAppointment}/>} {taskModal&&<TaskModal initial={taskModal} branches={avBranches} users={avUsers} leads={leads} profile={profile} onClose={()=>setTaskModal(null)} onSave={saveTask}/>} {boardModal&&<BoardModal initial={boardModal} branches={avBranches} profile={profile} onClose={()=>setBoardModal(null)} onSave={saveBoard}/>} {cardModal&&<BoardCardModal initial={cardModal} lists={boardLists.filter(l=>l.board_id===(cardModal.board_id||selectedBoard))} users={users} checks={cardChecks.filter(c=>c.card_id===cardModal.id)} comments={cardComments.filter(c=>c.card_id===cardModal.id)} profile={profile} onClose={()=>setCardModal(null)} onSave={saveBoardCard} onDelete={delBoardCard} onWorkflow={boardWorkflow} toggleCheck={toggleCheck} updateCheckText={updateCheckText} deleteCheck={deleteCheck} addComment={addComment} updateComment={updateComment} deleteComment={deleteComment}/>} {passwordModal&&<PasswordModal onClose={()=>setPasswordModal(null)} onSave={changeOwnPassword}/>}</div>}
...
function BoardPage({boards,branches,lists,cards,checks,comments,users,profile,selectedBoard,setSelectedBoard,setBoardModal,setCardModal,addList,renameList,delList,delBoard,moveCard,dragCard,reloadBoardById}){
 const[listName,setListName]=useState('');
 const isStaff=['sale','pt','reception'].includes(profile.role);
 const normalize=v=>String(v||'').toLocaleLowerCase('vi-VN').normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/đ/g,'d').replace(/[^a-z0-9]/g,'');
 const branchKey=name=>normalize(name).replace(/^(akcfitness|akc|fitness)/,'');
 const activeBranches=branches.filter(br=>boards.some(b=>b.branch_id===br.id));
 const primaryBoardForBranch=branchId=>{
  const options=boards.filter(b=>b.branch_id===branchId);
  const br=branches.find(x=>x.id===branchId);
  const key=branchKey(br?.name);
  return [...options].sort((a,b)=>{
   const score=x=>{const n=normalize(x.name);return(n.includes('coso')?100:0)+(key&&n.includes(key)?50:0)-Number(x.position||0)/1000};
   return score(b)-score(a);
  })[0];
 };
 const globalBoard=boards.filter(b=>!b.branch_id).sort((a,b)=>Number(a.position||0)-Number(b.position||0))[0];
 const selected=boards.find(b=>b.id===selectedBoard);
 const initialScope=isStaff?profile.branch_id:(selected?.branch_id||activeBranches[0]?.id||(globalBoard?'__global__':''));
 const[scope,setScope]=useState(initialScope);
 const board=scope==='__global__'?globalBoard:primaryBoardForBranch(scope);
 useEffect(()=>{
  const allowedScopes=isStaff?[profile.branch_id]:activeBranches.map(b=>b.id);
  const validScope=scope==='__global__'?!isStaff&&!!globalBoard:allowedScopes.includes(scope);
  const nextScope=validScope?scope:(isStaff?profile.branch_id:(activeBranches[0]?.id||(globalBoard?'__global__':'')));
  if(nextScope!==scope)setScope(nextScope);
  const target=nextScope==='__global__'?globalBoard:primaryBoardForBranch(nextScope);
  if(target&&target.id!==selectedBoard){setSelectedBoard(target.id);reloadBoardById?.(target.id,isStaff?profile.branch_id:undefined);}
 },[boards.length,branches.length,profile.id]);
 if(!boards.length)return <section className='board-empty'><h2>BOARD AKC</h2><p>Tạo Board nội bộ để quản lý nhân sự, phòng ban, checklist ngày/tuần/tháng và hội thoại trong từng thẻ.</p>{!isStaff&&<button className='primary' onClick={()=>setBoardModal({})}>+ Tạo board đầu tiên</button>}</section>;
 if(!board)return <section className='board-empty'><h2>Chưa có Board cho cơ sở này</h2><p>Vui lòng chọn cơ sở khác hoặc tạo Board mới.</p></section>;
 const bl=lists.filter(l=>l.board_id===board.id);
 const cardsByList=cards.reduce((acc,c)=>{(acc[c.list_id]||(acc[c.list_id]=[])).push(c);return acc},{});
 const checkStats=checks.reduce((acc,c)=>{const stat=acc[c.card_id]||(acc[c.card_id]={total:0,done:0});stat.total++;if(c.done)stat.done++;return acc},{});
 const commentCount=comments.reduce((acc,c)=>{acc[c.card_id]=(acc[c.card_id]||0)+1;return acc},{});
 const changeScope=branchId=>{
  setScope(branchId);
  const target=branchId==='__global__'?globalBoard:primaryBoardForBranch(branchId);
  if(target){setSelectedBoard(target.id);reloadBoardById?.(target.id,isStaff?profile.branch_id:undefined);}
 };
 return <section className='akc-board' style={{background:board.background||DEFAULT_BOARD_BG}}><div className='board-bar'><div className='board-scope'><label><span>Cơ sở</span><select value={scope} onChange={e=>changeScope(e.target.value)}>{!isStaff&&globalBoard&&<option value='__global__'>Ban điều hành</option>}{activeBranches.map(br=><option key={br.id} value={br.id}>{br.name}</option>)}</select></label><small>Board: <b>{board.name}</b></small>{!isStaff&&<><button onClick={()=>setBoardModal(board)}>Sửa board</button><button className='danger' onClick={()=>delBoard(board.id)}>Xóa</button></>}</div>{!isStaff&&<form onSubmit={e=>{e.preventDefault();addList(board.id,listName);setListName('')}}><input value={listName} onChange={e=>setListName(e.target.value)} placeholder='+ Thêm danh sách khác'/><button>Thêm</button></form>}</div><div className='board-canvas'>{bl.map(l=>{const cs=cardsByList[l.id]||[];return <div className='board-list' key={l.id} onDragOver={e=>e.preventDefault()} onDrop={()=>!isStaff&&dragCard.current&&moveCard(dragCard.current,l.id)}><div className='list-head'><b>{l.name}</b>{!isStaff&&<span><button onClick={()=>renameList(l)}>•••</button><button onClick={()=>delList(l.id)}>×</button></span>}</div>{cs.map(c=>{const stat=checkStats[c.id]||{total:0,done:0};return <div className='board-card' key={c.id} draggable={!isStaff} onDragStart={()=>dragCard.current=c.id} onClick={()=>setCardModal(c)}>{c.cover_image&&<img src={c.cover_image} loading='lazy' decoding='async' alt='cover'/>}<b>{c.title}</b>{c.description&&<p>{c.description.slice(0,90)}{c.description.length>90?'...':''}</p>}<div className='card-meta'>{c.label&&<span className='badge'>{c.label}</span>}{c.due_date&&<span>🕒 {c.due_date}</span>}{stat.total>0&&<span>☑ {stat.done}/{stat.total}</span>}{commentCount[c.id]>0&&<span>💬 {commentCount[c.id]}</span>}</div></div>})}{!isStaff&&<button className='add-card' onClick={()=>setCardModal({board_id:board.id,list_id:l.id})}>+ Thêm thẻ</button>}</div>})}</div></section>}
function BoardModal({initial,branches,profile,onClose,onSave}){const isStaff=['sale','pt','reception'].includes(profile.role);const[f,setF]=useState({name:'',branch_id:isStaff?profile.branch_id:(branches[0]?.id||''),background:DEFAULT_BOARD_BG,...initial});return <div className='modal dark-modal'><form className='modal-card' onSubmit={e=>{e.preventDefault();onSave(f)}}><div className='modal-head'><h3>{f.id?'Sửa board':'Tạo board'}</h3><button type='button' onClick={onClose}>×</button></div><label>Tên board<input value={f.name||''} onChange={e=>setF({...f,name:e.target.value})} required/></label><label>Cơ sở áp dụng<select disabled={isStaff} value={f.branch_id||''} onChange={e=>setF({...f,branch_id:e.target.value})}><option value=''>Toàn hệ thống / Admin</option>{branches.map(b=><option key={b.id} value={b.id}>{b.name}</option>)}</select></label><label>Background<input value={f.background||''} onChange={e=>setF({...f,background:e.target.value})} placeholder='linear-gradient(...) hoặc link ảnh'/></label><button className='primary full'>Lưu board</button></form></div>}
function BoardCardModal({initial,lists,users,checks,comments,profile,onClose,onSave,onDelete,onWorkflow,toggleCheck,updateCheckText,deleteCheck,addComment,updateComment,deleteComment}){
 ...
 const[f,setF]=useState(()=>({...{title:'',description:'',cover_image:'',label:'',recurrence_cycle:'none',owner_id:profile.id,reviewer_id:'',base_kpi_points:30,result_url:'',workflow_status:'Chưa bắt đầu',due_date:'',list_id:lists[0]?.id||'',checklist_text:''},...initial,checklist_text:''}));
 const set=(k,v)=>setF({...f,[k]:v});
 ...
 if(profile.role==='reception')return <div className='modal board-modal light-card-modal' onMouseDown={onClose}><section className='modal-card reception-checklist' onMouseDown={e=>e.stopPropagation()}><div className='modal-head'><div><h3>{f.title}</h3><p>{f.description||'Checklist công việc được giao'}</p></div><button type='button' onClick={onClose}>×</button></div><div className='check-title'><h4>Tiến độ</h4><span>{done}/{total} · {percent}%</span></div><div className='mini-progress'><i style={{width:percent+'%'}}></i></div><div className='checklist trello-checks compact-checks'>{checks.map(c=><label key={c.id} className='check-line reception-check'><input type='checkbox' checked={!!c.done} onChange={()=>toggleCheck(c.id,c.done)}/><span className={c.done?'check-text done':'check-text'}>{c.text}</span></label>)}{!checks.length&&<p>Thẻ này chưa có checklist.</p>}</div><button type='button' className='primary full' onClick={onClose}>Đóng</button></section></div>;
 return <div className='modal board-modal light-card-modal' onMouseDown={onClose}>
  <form className='modal-card board-card-detail trello-detail-60-40' onMouseDown={e=>e.stopPropagation()} onSubmit={e=>{e.preventDefault();onSave(f)}}>
   ...
   <div className='detail-left'>
    ...
    <div className='modal-grid compact-grid'><label>Danh sách<select value={f.list_id||''} onChange={e=>set('list_id',e.target.value)}>{lists.map(l=><option key={l.id} value={l.id}>{l.name}</option>)}</select></label><label>Người phụ trách<select value={f.owner_id||''} onChange={e=>set('owner_id',e.target.value)}><option value=''>Chưa gán</option>{users.map(u=><option key={u.id} value={u.id}>{u.full_name}</option>)}</select></label><label>Người nghiệm thu<select value={f.reviewer_id||''} onChange={e=>set('reviewer_id',e.target.value)}><option value=''>Manager/Admin bất kỳ</option>{users.filter(u=>['admin','manager'].includes(u.role)).map(u=><option key={u.id} value={u.id}>{u.full_name}</option>)}</select></label><label>Điểm KPI cơ sở<input type='number' min='0' max='1000' value={f.base_kpi_points||30} onChange={e=>set('base_kpi_points',e.target.value)}/></label><label>Ngày<input type='date' value={f.due_date||''} onChange={e=>set('due_date',e.target.value)}/></label><label>Nhãn<input value={f.label||''} onChange={e=>set('label',e.target.value)} placeholder='CEO / Checklist ngày...'/></label><label>Chu kỳ checklist<select value={f.recurrence_cycle||'none'} onChange={e=>set('recurrence_cycle',e.target.value)}><option value='none'>Không tự reset</option><option value='daily'>Hàng ngày · 21:00</option><option value='weekly'>Hàng tuần · Chủ nhật 21:00</option><option value='monthly'>Hàng tháng · ngày cuối tháng 21:00</option></select></label></div>{f.recurrence_cycle&&f.recurrence_cycle!=='none'&&<small className='cycle-hint'>Hệ thống sẽ gửi báo cáo Telegram trước khi reset checklist theo chu kỳ đã chọn.</small>}
    ...
    {f.id&&<div className='workflow-box'><div className='workflow-head'><b>Tiến độ & nghiệm thu</b><span>{f.workflow_status||'Chưa bắt đầu'}</span></div>{f.review_note&&<p className='decision-note'>Ý kiến nghiệm thu: {f.review_note}</p>}{f.earned_kpi_points>0&&<p className='kpi-earned'>+{f.earned_kpi_points} điểm KPI</p>}{f.owner_id===profile.id&&['Chưa bắt đầu','Làm lại'].includes(f.workflow_status)&&<button type='button' onClick={()=>onWorkflow('start',f)}>Bắt đầu thực hiện</button>}{f.owner_id===profile.id&&['Đang thực hiện','Làm lại'].includes(f.workflow_status)&&<div className='result-submit'><input value={f.result_url||''} onChange={e=>set('result_url',e.target.value)} placeholder='Dán link Drive hoặc tài liệu kết quả'/><button type='button' className='primary' onClick={()=>onWorkflow('submit',f,{result_url:f.result_url})}>Gửi nghiệm thu</button></div>}{['admin','manager'].includes(profile.role)&&f.workflow_status==='Chờ nghiệm thu'&&<div className='review-actions'><button type='button' className='primary' onClick={()=>onWorkflow('approve',f,{note:prompt('Ghi chú nghiệm thu (không bắt buộc)')||''})}>Duyệt đạt</button><button type='button' className='danger' onClick={()=>{const note=prompt('Lý do yêu cầu làm lại');if(note?.trim())onWorkflow('redo',f,{note})}}>Yêu cầu làm lại</button></div>}{f.result_url&&<a href={f.result_url} target='_blank' rel='noreferrer'>Mở kết quả đã nộp</a>}</div>}
