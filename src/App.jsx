...
try{
 ...
 async function loadBoard(boardIdOverride,branchFilter){// Fix: load cards theo board đang chọn, tránh load toàn bộ
  const[{data:bd},{data:bl}]=await Promise.all([supabase.from('boards').select('*').order('position',{ascending:true}).order('created_at',{ascending:true}),supabase.from('board_lists').select('*').order('position',{ascending:true})]);setBoards(bd||[]);const allBoards=bd||[];// Fix: với sale/PT, ưu tiên board thuộc đúng branch của user
  const branchBoards=branchFilter?allBoards.filter(b=>b.branch_id===branchFilter):allBoards;const defaultBoard=(branchBoards[0]||allBoards[0])?.id||'';const targetBoard=boardIdOverride||(selectedBoard&&allBoards.find(b=>b.id===selectedBoard)?selectedBoard:null)||defaultBoard||'';if(targetBoard&&!(selectedBoard&&(bd||[]).find(b=>b.id===selectedBoard)))setSelectedBoard(targetBoard);if(!targetBoard)return;const{data:bc}=await supabase.from('board_cards').select('*').eq('board_id',targetBoard).order('position',{ascending:true}).order('created_at',{ascending:true});setBoardLists(bl||[]);setBoardCards(bc||[]);
...
const{data:oldChecks}=await supabase.from('card_checklists').select('text,position').eq('card_id',cardId);const old=oldChecks||[];const newLines=lines.filter(l=>!old.find(o=>String(o.text).trim().toLowerCase()===l.toLowerCase()));if(newLines.length){const maxPos=old.length?Math.max(...old.map(o=>o.position||0))+1:0;await Promise.all(newLines.map((text,i)=>supabase.from('card_checklists').insert({card_id:cardId,text,position:maxPos+i,created_by:profile.id})));}}}setCardModal(null);loadBoard()}
 async function openCardModal(card){// Fix 2: Lazy load full checklist text khi mở modal
  setCardModal(card);
  const{data:fullChecks}=await supabase.from('card_checklists').select('*').eq('card_id',card.id).order('position',{ascending:true});
