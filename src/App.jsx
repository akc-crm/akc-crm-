...
function App(){
 ...
 const[filter,setFilter]=useState({branch_id:'',source:'',owner_id:'',date:'',q:''});
...
try{
 ...
 if(p?.role==='manager'&&p.branch_id)sq=sq.eq('branch_id',p.branch_id);
 const{data:ps}=await sq;
 setPtShows(ps||[]);
}catch(_e){setPtShows([])}
const{data:bd}=await supabase.from('boards').select('*').order('position',{ascending:true}).order('created_at',{ascending:true});setBoards(bd||[]);if((bd||[]).length&&!selectedBoard)setSelectedBoard((bd||[])[0].id);const{data:bl}=await supabase.from('board_lists').select('*').order('position',{ascending:true});setBoardLists(bl||[]);const{data:bc}=await supabase.from('board_cards').select('*').order('position',{ascending:true}).order('created_at',{ascending:true});setBoardCards(bc||[]);const{data:cc}=await supabase.from('card_checklists').select('*').order('position',{ascending:true});setCardChecks(cc||[]);const{data:cm}=await supabase.from('card_comments').select('*').order('created_at',{ascending:true});setCardComments(cm||[])}if(!silent)setLoading(false)}
 useEffect(()=>{load(false)},[session?.user?.id]);
 useEffect(()=>{if(!session?.user)return;const c=supabase.channel('akc-v8').on('postgres_changes',{event:'*',schema:'public',table:'leads'},load).on('postgres_changes',{event:'*',schema:'public',table:'profiles'},load).on('postgres_changes',{event:'*',schema:'public',table:'branches'},load).on('postgres_changes',{event:'*',schema:'public',table:'operation_tasks'},load).on('postgres_changes',{event:'*',schema:'public',table:'pt_checklists'},load).on('postgres_changes',{event:'*',schema:'public',table:'boards'},load).on('postgres_changes',{event:'*',schema:'public',table:'board_lists'},load).on('postgres_changes',{event:'*',schema:'public',table:'board_cards'},load).on('postgres_changes',{event:'*',schema:'public',table:'card_checklists'},load).on('postgres_changes',{event:'*',schema:'public',table:'card_comments'},load).subscribe();return()=>supabase.removeChannel(c)},[session?.user?.id]);
 ...
 const rows=leads.filter(l=>(!filter.branch_id||l.branch_id===filter.branch_id)&&(!filter.source||l.source===filter.source)&&(!filter.owner_id||l.owner_id===filter.owner_id)&&(!filter.date||String(l.created_at).slice(0,10)===filter.date)&&(!filter.q||String((l.name||'')+' '+(l.phone||'')+' '+(l.note||'')).toLowerCase().includes(filter.q.toLowerCase())));
 ...
 async function savePtShow(f){
  ...
  if(profile.role==='manager'&&profile.branch_id&&f.branch_id&&f.branch_id!==profile.branch_id)return alert('Manager chỉ được tạo/sửa show của cơ sở mình.');
  const pt=profile.role==='pt'?profile:(users.find(u=>u.id===(f.pt_id||profile.id))||profile);
  const branchId=profile.role==='pt'?profile.branch_id:(profile.role==='manager'?profile.branch_id:(f.branch_id||profile.branch_id||null));
 ...
 async function rejectPtShow(id){
  const show=ptShows.find(x=>x.id===id);
  if(!show||!(isAdmin||(isManager&&(!profile.branch_id||show.branch_id===profile.branch_id))))return alert('Chỉ manager/admin đúng quyền mới được từ chối show này.');
 ...
 async function markKiotInvoice(show){
  ...
  if(isManager&&profile.branch_id&&show.branch_id!==profile.branch_id)return alert('Manager chỉ được xử lý show của cơ sở mình.');
...
function TeachingShows({shows,branches,users,profile,edit,complete,approve,reject,markInvoice}){
