...
function BoardPage({boards,branches,lists,cards,checks,comments,users,profile,selectedBoard,setSelectedBoard,setBoardModal,setCardModal,addList,renameList,delList,delBoard,moveCard,dragCard,reloadBoardById}){
 ...
 const isStaff=['sale','pt'].includes(profile.role);
 ...
 if(!boards.length)return <section className='board-empty'><h2>BOARD AKC</h2><p>Tạo Board nội bộ để quản lý nhân sự, phòng ban, checklist ngày/tuần/tháng và hội thoại trong từng thẻ.</p>{!isPT&&<button className='primary' onClick={()=>setBoardModal({})}>+ Tạo board đầu tiên</button>}</section>;
 ...
 return <section className='akc-board' style={{background:board.background||DEFAULT_BOARD_BG}}><div className='board-bar'><div className='board-scope'><label><span>Cơ sở</span><select value={scope} onChange={e=>changeScope(e.target.value)}>{!isStaff&&globalBoard&&<option value='__global__'>Ban điều hành</option>}{activeBranches.map(br=><option key={br.id} value={br.id}>{br.name}</option>)}</select></label><small>Board: <b>{board.name}</b></small>{!isPT&&<><button onClick={()=>setBoardModal(board)}>Sửa board</button><button className='danger' onClick={()=>delBoard(board.id)}>Xóa</button></>}</div>{!isPT&&<form onSubmit={e=>{e.preventDefault();addList(board.id,listName);setListName('')}}><input value={listName} onChange={e=>setListName(e.target.value)} placeholder='+ Thêm danh sách khác'/><button>Thêm</button></form>}</div><div className='board-canvas'>{bl.map(l=>{const cs=cardsByList[l.id]||[];return <div className='board-list' key={l.id} onDragOver={e=>e.preventDefault()} onDrop={()=>dragCard.current&&moveCard(dragCard.current,l.id)}><div className='list-head'><b>{l.name}</b><span><button onClick={()=>renameList(l)}>•••</button><button onClick={()=>delList(l.id)}>×</button></span></div>{cs.map(c=>{const stat=checkStats[c.id]||{total:0,done:0};return <div className='board-card' key={c.id} draggable onDragStart={()=>dragCard.current=c.id} onClick={()=>setCardModal(c)}>{c.cover_image&&<img src={c.cover_image} loading='lazy' decoding='async' alt='cover'/>}<b>{c.title}</b>{c.description&&<p>{c.description.slice(0,90)}{c.description.length>90?'...':''}</p>}<div className='card-meta'>{c.label&&<span className='badge'>{c.label}</span>}{c.due_date&&<span>🕒 {c.due_date}</span>}{stat.total>0&&<span>☑ {stat.done}/{stat.total}</span>}{commentCount[c.id]>0&&<span>💬 {commentCount[c.id]}</span>}</div></div>})}<button className='add-card' onClick={()=>setCardModal({board_id:board.id,list_id:l.id})}>+ Thêm thẻ</button></div>})}</div></section>}
...
function BoardCardModal({initial,lists,users,checks,comments,profile,onClose,onSave,onDelete,toggleCheck,updateCheckText,deleteCheck,addComment,updateComment,deleteComment}){
 ...
 const[f,setF]=useState(()=>({...{title:'',description:'',cover_image:'',label:'',recurrence_cycle:'none',owner_id:profile.id,due_date:'',list_id:lists[0]?.id||'',checklist_text:''},...initial,checklist_text:''}));
 const set=(k,v)=>setF({...f,[k]:v});
