...
try{
 ...
 async function delBoard(id){if(!(isAdmin||isManager))return alert('Chỉ admin/manager được xóa board');if(confirm('Xóa board này và toàn bộ danh sách/thẻ?')){const{error}=await supabase.from('boards').delete().eq('id',id);if(error)alert(error.message);setSelectedBoard('');loadBoard()}}
 ...
 async function delBoardList(id){if(confirm('Xóa danh sách và các thẻ bên trong?')){const{error}=await supabase.from('board_lists').delete().eq('id',id);if(error)alert(error.message);loadBoard()}}
...
const{data:oldChecks}=await supabase.from('card_checklists').select('text,position').eq('card_id',cardId);const old=oldChecks||[];const newLines=lines.filter(l=>!old.find(o=>String(o.text).trim().toLowerCase()===l.toLowerCase()));if(newLines.length){const maxPos=old.length?Math.max(...old.map(o=>o.position||0))+1:0;await Promise.all(newLines.map((text,i)=>supabase.from('card_checklists').insert({card_id:cardId,text,position:maxPos+i,created_by:profile.id})));}}}setCardModal(null);loadBoard()}
 ...
 async function delBoardCard(id){if(confirm('Xóa thẻ này?')){const{error}=await supabase.from('board_cards').delete().eq('id',id);if(error)alert(error.message);setCardModal(null);loadBoard()}}
 ...
 async function deleteComment(id){if(!confirm('Xóa bình luận này?'))return;const prev=cardComments;setCardComments(c=>c.filter(x=>x.id!==id));const{error}=await supabase.from('card_comments').delete().eq('id',id);if(error){alert(error.message);setCardComments(prev);}}
