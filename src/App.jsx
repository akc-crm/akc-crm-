...
try{
 ...
 async function loadBoard(){// Fix 2: Lazy load - chỉ load board structure, không load toàn bộ 874 checklist
  const[{data:bd},{data:bl},{data:bc},{data:cm}]=await Promise.all([supabase.from('boards').select('*').order('position',{ascending:true}).order('created_at',{ascending:true}),supabase.from('board_lists').select('*').order('position',{ascending:true}),supabase.from('board_cards').select('*').order('position',{ascending:true}).order('created_at',{ascending:true}),supabase.from('card_comments').select('*').order('created_at',{ascending:true})]);setBoards(bd||[]);if((bd||[]).length&&!selectedBoard)setSelectedBoard((bd||[])[0].id);setBoardLists(bl||[]);setBoardCards(bc||[]);setCardComments(cm||[]);
 ...
 useEffect(()=>{if(!session?.user)return;
  ...
  const reloadBoard=()=>{if(page!=='BOARD')return;clearTimeout(boardDebounceTimer);boardDebounceTimer=setTimeout(()=>loadBoard(),600);};
