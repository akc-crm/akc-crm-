...
function App(){
 ...
 useEffect(()=>{if(!session?.user)return;
  ...
  // Chỉ reload cấu trúc khi Board/List đổi; Card/Comment được cập nhật theo payload để tránh bão truy vấn.
  ...
  const c=supabase.channel('akc-v10').on('postgres_changes',{event:'*',schema:'public',table:'leads'},reloadLeads).on('postgres_changes',{event:'*',schema:'public',table:'profiles'},reloadProfiles).on('postgres_changes',{event:'*',schema:'public',table:'branches'},reloadBranches).on('postgres_changes',{event:'*',schema:'public',table:'operation_tasks'},reloadTasks).on('postgres_changes',{event:'*',schema:'public',table:'pt_checklists'},reloadPtShows).on('postgres_changes',{event:'*',schema:'public',table:'boards'},reloadBoardStructure).on('postgres_changes',{event:'*',schema:'public',table:'board_lists'},reloadBoardStructure).on('postgres_changes',{event:'*',schema:'public',table:'board_cards'},applyCardChange).on('postgres_changes',{event:'*',schema:'public',table:'card_comments'},applyCommentChange).subscribe();return()=>{clearTimeout(boardDebounceTimer);supabase.removeChannel(c)}},[session?.user?.id,page,selectedBoard]);
